...
\title[Fixed points of strictly non-Volterra operators]{Fixed points of strictly non-Volterra quadratic stochastic operators in four or more types}
...
Problem 11 of \cite{GR09} asks: \emph{is Theorem 6 true for $m\ge4$?} The two clauses about the fixed point fail at different thresholds, and we determine both.
...
The operator of this section was introduced by Jamilov \cite{Jam17}; we describe it by its coefficient table, which is the form that \Cref{thm:swap} generalises. Let $F=\{1,2,3\}$ and define the coefficients of $W$ on five types as follows (all coefficients not listed, and their symmetric copies $P_{ji,k}=P_{ij,k}$, are zero):
\begin{align*}
...
Jamilov \cite{Jam17} obtains the count by subtracting pairs of coordinate equations, which gives $x_1=x_2=x_3$ and $x_4=x_5$; this is the argument of \Cref{thm:swap} below in the present special case. We record the eigenvectors, and a second proof of the count that determines the whole solution set of the polynomial system.
...
So the four-type fixed point has index $+1$, as the Lefschetz number requires of a unique nondegenerate fixed point. In constrained searches over four-type operators with $V(x)=x$ imposed, the smallest value of $3+4e_3-12\det A$ observed was about $3.14$, at a point of the form $(a,a,a,a^2)$ with $a=(\sqrt{13}-3)/2$, and the largest value of $\det A$ about $0.056$, against the $0.25$ that a negative index would need with $e_3=0$. Without the fixed-point condition the quantity can vanish, so both (ii) and \Cref{thm:four} must use $V(x)=x$, which enters through the cap \eqref{eq:cap}.
...
This paper is a product of an ongoing project on the use of AI systems for research mathematics, provisionally called Math Brain, which studies how to make such systems better at solving open problems; the project and its tooling will be released publicly in due course. The mathematics, the literature search, the computations, the Lean formalisation and the text were produced by AI systems (Claude Opus 5 and Claude Fable 5.1, Anthropic; Codex with GPT-6 Astra, OpenAI), working from the problem list \cite{GR09}. Three independent adversarial reviews, also by Claude Fable 5.1, re-derived every claim; the index identities \eqref{eq:index}, \Cref{prop:index}(i), the constrained searches of \Cref{sec:index} and the statement of \Cref{thm:swap} originate in those reviews, as do the references \cite{Jam16,Jam17}. \Cref{thm:four} and \Cref{prop:index}(ii) originate in a Codex run on the project's records. The finite verifications and \Cref{thm:four} are machine-checked in Lean~4 (\Cref{sec:lean}). The arguments have not been refereed by a human mathematician. The author set the goals of the project, commissioned and directed the work and approved the release; the author did not carry out the mathematics, the verification, the computations or the writing, and takes responsibility for the decision to publish.
